 ...
 * File: VehicleHashSetImplTest.java
 * This file contains the VehicleHashSetImplTest class which is responsible for
 * testing methods added to the VehicleHashSetImplTest class.
 */
...
public class VehicleHashSetImplTest {
  ...
  /**
   * Vehicle objects added to HashSet
  ...
  public void removeObjectTest() throws Exception {
    ...
    ArrayList<Vehicle> al = createList();
    //Checks "happy path"
    assertEquals(6, al.size());//HashSet size before remove()
    this.impl.remove(car1);
    al = createList();
    assertEquals(5, al.size());//HashSet size after remove()
  ...
  public void isPresentObjectTest() throws Exception {
    ...
    this.impl.emptyList();
    addAllVehicles();
    Vehicle car7 = new Vehicle();//dummy object not stored in HashSet
